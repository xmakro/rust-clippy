...
///
/// Equivalent to [`Span::in_external_macro`], but with a single-slot cache. Many lints gate on
/// this check for the node currently being visited, and the combined pass runs all their
/// `check_*` callbacks on that node back to back, so the redundant queries are consecutive. The
/// result is a pure function of the `SyntaxContext` (expansion data and the `SourceMap` are
