...
impl<'tcx> LateLintPass<'tcx> for QuestionMark {
    fn check_stmt(&mut self, cx: &LateContext<'tcx>, stmt: &'tcx Stmt<'_>) {
        // Both checks below only match `let` statements; check this before the
        // more expensive lint level and const context queries.
    ...
    fn check_expr(&mut self, cx: &LateContext<'tcx>, expr: &'tcx Expr<'_>) {
        ...
        // All checks below only match `if` or `match` expressions; check this
        // before the more expensive lint level and const context queries.
