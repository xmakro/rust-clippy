...
/// Checks if the `match` expression contains comments which are not inside the arm bodies, as
/// applying the suggestion would lose them.
...
pub(crate) fn check<'tcx>(cx: &LateContext<'tcx>, ex: &'tcx Expr<'_>, arms: &'tcx [Arm<'_>], expr: &'tcx Expr<'_>) {
        ...
        && !matches!(arm1.pat.kind, PatKind::Or(..))
    {
        ...
        if *typeck.expr_ty(ex).peel_refs().kind() != ty::Bool || is_lint_allowed(cx, MATCH_BOOL, ex.hir_id) {
            ...
            // Comment extraction requires tokenizing the source of the whole `match`, so only do it
            // once the lint is guaranteed to be emitted.
