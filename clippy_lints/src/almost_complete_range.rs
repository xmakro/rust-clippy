...
pub(crate) fn check_expr(cx: &EarlyContext<'_>, e: &Expr, msrv: &MsrvStack) {
...
pub(crate) fn check_pat(cx: &EarlyContext<'_>, p: &Pat, msrv: &MsrvStack) {
        ...
        span_lint_and_then(
            ...
            |diag| {
                diag.span_suggestion(
                    kind.span,
                    "use an inclusive range",
                    if msrv.meets(msrvs::RANGE_INCLUSIVE) {
                        "..=".to_owned()
                    } else {
                        "...".to_owned()
                    },
