...
/// The lint can only fire on arguments of the form `&mut ...`, so the type and path
/// lookups below are only done once such an argument has been found.
...
fn check_arguments<'tcx>(
    ...
    // lazy: rendering the path is relatively expensive, only do it when the lint fires
    name: &dyn Fn() -> String,
    fn_kind: &str,
) {
