...
fn check_collapsible_else_if(cx: &LateContext<'_>, then_span: Span, else_block: &Block<'_>, lint_commented_code: bool) {
    if let Some(else_) = expr_block(else_block)
        && !else_.span.from_expansion()
        && let ExprKind::If(else_if_cond, ..) = else_.kind
        && check_significant_tokens_and_expect_attrs(
            cx,
            else_block,
            else_,
            sym::collapsible_else_if,
            lint_commented_code,
        )
    {
...
fn check_collapsible_if_if(
    ...
    if let Some(inner) = expr_block(then)
        ...
        && eligible_condition(cx, check_inner, msrv)
        && expr.span.eq_ctxt(inner.span)
        && check_significant_tokens_and_expect_attrs(cx, then, inner, sym::collapsible_if, lint_commented_code)
...
fn eligible_condition(cx: &LateContext<'_>, cond: &Expr<'_>, msrv: Msrv) -> bool {
    !matches!(cond.kind, ExprKind::Let(..)) || can_use_if_let_chains(cx, msrv)
}
...
fn check_significant_tokens_and_expect_attrs(
    ...
    match cx.tcx.hir_attrs(inner_if_expr.hir_id) {
                ...
                && [expected_lint_name, sym::style, sym::all].contains(&lint_name.ident.name) =>
        {
            ...
            !span_contains_non_whitespace(cx, span_before_attr, lint_commented_code)
                && !span_contains_non_whitespace(cx, span_after_attr, lint_commented_code)
        },
...
pub(crate) fn check<'tcx>(cx: &LateContext<'tcx>, expr: &'tcx Expr<'tcx>, msrv: Msrv, lint_commented_code: bool) {
        ...
        && !expr.span.from_expansion()
    {
        ...
        {
            check_collapsible_else_if(cx, then.span, else_, lint_commented_code);
        } else if else_.is_none()
            && eligible_condition(cx, cond, msrv)
            && let ExprKind::Block(then, None) = then.kind
        {
            check_collapsible_if_if(cx, expr, cond, then, msrv, lint_commented_code);
        }
    }
}
