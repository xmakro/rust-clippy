...
fn visit_type(ty: &Ty, cx: &EarlyContext<'_>, reason: &'static str) {
    match ty.kind {
        ...
        TyKind::Array(ref ty, _) | TyKind::Slice(ref ty) => {
            visit_type(ty, cx, reason);
        },
        TyKind::Tup(ref tup) => {
            for tup_ty in tup {
                visit_type(tup_ty, cx, reason);
            }
        },
        ...
        TyKind::Ref(ref optional_lifetime, ref borrow_type) => {
            ...
            visit_type(&borrow_type.ty, cx, reason);
        },
        _ => {},
    }
}

pub(crate) fn check_item(cx: &EarlyContext<'_>, item: &Item, msrv: &MsrvStack) {
    ...
    if !item.span.from_expansion() {
        if let ItemKind::Const(ConstItem { ty: ref var_type, .. }) = item.kind {
            visit_type(var_type, cx, "constants have by default a `'static` lifetime");
            // Don't check associated consts because `'static` cannot be elided on those (issue
            // #2438)
        }

        if let ItemKind::Static(StaticItem { ty: ref var_type, .. }) = item.kind {
            visit_type(var_type, cx, "statics have by default a `'static` lifetime");
        }
    }
}
