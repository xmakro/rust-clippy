...
impl<'tcx> LateLintPass<'tcx> for ManualClamp {
    fn check_expr(&mut self, cx: &LateContext<'tcx>, expr: &'tcx Expr<'tcx>) {
        // All patterns below only match `if`, `match`, method call or call
        // expressions; check this before the more expensive const context query.
        if matches!(
            expr.kind,
    ...
    fn check_block(&mut self, cx: &LateContext<'tcx>, block: &'tcx Block<'tcx>) {
        // The pattern matched by `is_two_if_pattern` starts with an `if`
        // expression statement; check this before the more expensive const
        // context query.
        if !block
            .stmts
            .iter()
...
fn is_max_min_pattern<'tcx>(cx: &LateContext<'tcx>, expr: &'tcx Expr<'tcx>) -> Option<ClampSuggestion<'tcx>> {
    if let ExprKind::MethodCall(seg_second, receiver, [arg_second], _) = expr.kind
        ...
        // Check the method names before the more expensive type queries.
